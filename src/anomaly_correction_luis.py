...
class AnomalyCorrection:
  ...
  def set_diffusion_model(self,
    ...
    """
    assert self.ddpm_scheduler is not None, 'The scheduler must be set'
    ...
    if self.ddpm_model is None and model_filename is None:
      self.ddpm_model = NoisePredictor(time_dim=time_dim_embedding,
                                      ...
                                      unet=unet)
      self.diffusion = DDPM(self.ddpm_scheduler, self.ddpm_model)
    else:
      self.diffusion = DDPM(self.ddpm_scheduler)
