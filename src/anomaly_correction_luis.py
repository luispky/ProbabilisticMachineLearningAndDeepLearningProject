...
class AnomalyCorrection:
    ...
    def difussion_correction(self, data_to_inpaint,  # dictionary with x and y
        ...
        #!INPAINTING------------------------------------------------------------------
        # todo: transform the data to extract in dictionary form
        ...
        for mask in masks:
            ...
            inpainted_instance_logits = diffusion.inpaint(diffusion.ema_model, x, mask)
        ...
        y = y.numpy().squeeze()  # todo: create a np.ones(len(anomalies))
