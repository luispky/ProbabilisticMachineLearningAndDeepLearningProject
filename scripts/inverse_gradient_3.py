...
from misc import cprint, bcolors, Probabilities
...
class InverseGradient:
    ...
    def run(self, x, n_values, eta=0.01, n_iter=300, threshold=0.1):
        """
...
def main(n_data=1000, n_values=(2, 2, 4), threshold=3.5,
         hidden=2, n_epochs=2000, lr=0.1, weight_decay=1e-3, momentum=0.9,
         n_data_examples=15, n_anomalies=5):
    np.random.seed(42)
    ...
    for i in range(n_anomalies):
        ...
        # run the inverse gradient algorithm
        new_anomaly, loss = method.run(anomaly, n_values, threshold=0.1)
