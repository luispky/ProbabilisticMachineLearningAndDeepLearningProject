...
def main_sum_categorical_data():
    ...
    if save_model:
        ...
        model = NoisePredictor(time_dim=time_dim_embedding,
                            ...
                            unet=unet)
        # DDPM model
        diffusion = DDPM(scheduler, model)
    ...
    #!DDPM: TRAINING AND SAMPLING-------------------------------------------------------
        ...
        diffusion.save_model_pickle(f"diffusion_model_{experiment_number}.pkl")
    
        # DDPM sampling
        sampled_logits = diffusion.sample(model=diffusion.ema_model, 
                                          samples=samples)[0]
    ...
    if inpaiting:
        ...
        size = 1500
        dataset_generator = SumCategoricalDataset(size, structure, threshold)
        _ = dataset_generator.generate_dataset(remove_anomalies=False, logits=True)
        dataset_shape = dataset_generator.get_dataset_shape()
        ...
        scheduler = LinearNoiseScheduler(noise_timesteps=noise_time_steps,
                                         dataset_shape=dataset_shape)
        diffusion = DDPM(scheduler)
        diffusion.load_model_pickle(f"diffusion_model_{experiment_number}.pkl")

        # inpaint the masked data: probabilities
        inpainted_data = diffusion.inpaint(model=diffusion.model,
                                           original=x,
                                           mask=mask, 
